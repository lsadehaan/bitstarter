...
var URL_DEFAULT = "http://evening-plateau-4069.herokuapp.com/";
...
var checkHtmlURL = function(URL, checksfile) {
	...
	if (result instanceof Error){
	    ...
	    if (retries < 3){
		retries = retries + 1;
		console.log(retries + " of 3 retries.");
		retry(3000); // try again after 3 sec.
	    }
	} else {
	    cheeryResult = cheerio.load(result);
	    var checks = loadChecks(checksfile).sort();
	    var out = {};
	    for(var ii in checks) {
		var present = cheeryResult(checks[ii]).length > 0;
		out[checks[ii]] = present;
	    }
	    console.log(out);
	}
    ...
    var result = rest.get(URL).on('complete', callback);
...
if(require.main == module) {
    program
        ...
        .option('-u, --url <url>', 'URL to html', clone(assertValidURL), URL_DEFAULT)
